(function () {
  ...
  var onSpecial = function (key) {
    media.special = [media.getSPM(new Date()),media[key + 'Playlist']];
  }
  ...
  var makePlaylistIndex = function () {
    var i;
    var plkeys = Object.keys(PLs);
    for (i = 0; i < plkeys.length; i += 1) {
        media[plkeys[i] + 'Playlist'] = makePlaylist(PLs[plkeys[i]]);
    }
    media.plkeys = plkeys;
  }
