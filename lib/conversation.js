(function() {
  ...
  require('amdefine')(module)(['events','./eventlog','util'],
	...
	function isWarning(string) {
	  return string.toLowerCase().match(/.*?warning.*?/) !== null;
	}

	function isGirls(string) {
	  return string.toLowerCase().match(/.*?girls.*?/) !== null;
	}
	...
	*/
	...
	Conversation.prototype.react = function(word) {
	  ...
	  if (word.toLowerCase().indexOf(this.nick.toLowerCase()) !== -1) {
	  	...
	  	} else if (isWarning(word)) {
	  	  this.emit('say', 'ok, starting the warning playlist...');
	  	  this.emit('warning');
		} else if (isGirls(word)) {
		  this.emit('say', 'ok, playing some girls...');
		  this.emit('girls');
