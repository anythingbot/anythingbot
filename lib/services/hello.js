      ...
      wss.on('connection', function (ws) {
        ...
        ws.on('message', function(data, flags) {
          ...
          if (data.content.slice(0,4) == "skip") {
              ...
              media.onRestart();
          } else if (data.content == "candidates") {
              d.content = JSON.stringify(media.candidates);
          } else if (data.content == "puzzle") {
              var now = new Date();
              var utc = new Date(now.getUTCFullYear(),
                  now.getUTCMonth(), now.getUTCDate(),
                  now.getUTCHours(), now.getUTCMinutes(),
                  now.getUTCSeconds());
              var puzzleID = media.getPuzzleForUTC(utc);
              d.content = puzzleID;
          } else if (data.content.slice(0,6) == "answer") {
              var now = new Date();
              var utc = new Date(now.getUTCFullYear(),
                  now.getUTCMonth(), now.getUTCDate(),
                  now.getUTCHours(), now.getUTCMinutes(),
                  now.getUTCSeconds());
              var puzzleID = media.getPuzzleForUTC(utc);
              var answer = data.content.slice(7);
              media.candidates[puzzleID] = answer;
              var works = media.checkSolution(puzzleID);
              var result;
              if (works) {
                  result = "solves " + puzzleID;
              } else {
                  result = "does not solve " + puzzleID;
              }
              d.content = "the answer " + answer + " " + result;
          } else if (data.content.slice(0,8) == "constant") {
              ;
              ...
              var i;
              d.content = '';
              for (i = 0; i < jobs.jobList.length; i += 1) {
                  var job = jobs.jobList[i];
                  d.content += 'job ' + i + ' status = ' + job.status + ';\n';
              }
            ...
            if (jobs.activeJobs.length == 0) {
              ...
              c = function () {
                if (job.status == "term") {
                    var msg = 'complete: job ' + jobID;
                    console.log(msg);
                    var d = {content: msg};
                    ws.send(JSON.stringify(d), function () { });
                } else {
                    var msg = 'in progress: job ' + jobID;
                    console.log(msg);
                    var d = {content: msg};
                    ws.send(JSON.stringify(d), function () { });
                    jobs.awsWork(jobID, c);
                }
              };
              jobs.awsWork(jobID, c);
              reply = 0;
