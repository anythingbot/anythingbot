(function() {
  ...
  define(deps, function( _, sentiment, config, events, eventlog, gh, templates,
	...
	var initialize;
	var initializeEvents; // init event handling
    ...
    // Check for a "vote started" comment posted by the bot.
    // @return (object|null)
    ...
    var Voting = function Voting(repo) {
	  ...
	  g_voteController = new VoteController(repo, g_votingConfig, g_starGazers,
			g_votesPerPR);
      g_templates = templates(g_votingConfig);
    ...
    postVoteStarted = function postVoteStarted(pr) {
      ...
      getVoteStartedComment(pr, function (err, comment) {
        ...
        makeComment(
          pr.number,
          g_templates.voteStartedComment + " " + pr.head.sha, function (err) {
			if (err) {
    ...
    getVoteStartedComment = function getVoteStartedComment(pr, cb) {
      ...
      for (var i = 0; i < pr.comments.length; i++) {
        ...
        var isVoteStarted =
          pr.comments[i].body.indexOf(g_templates.voteStartedComment) === 0;
    ...
    processPR = function processPR(pr, cb) {
      ...
      if (age / MINUTE < g_votingConfig.truePeriod) { return cb(null, false); }
      ...
      if (highestVote < g_votingConfig.guaranteedResult) { return cb(null, false); }
      // vote passes if yeas > nays
      var passes = g_voteController.doesVotePass(voteResults.positive,
        voteResults.negative);
      var endComment = g_templates.voteEndComment(passes, voteResults.positive,
    ...
    mergePR = function mergePR(pr, cb) {
      ...
      gh.pullRequests.get({
        ...
        if (res.mergeable === false) {
          ...
          return closePR(g_templates.couldntMergeWarning, pr, function () {
