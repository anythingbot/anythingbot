(function() {
  ...
  define(deps, function( _, sentiment, config, events, eventlog, gh, templates,
      VoteController
  ) {
    ...
    var Voting = function Voting(repo) {
      ...
      this.votingConfig.truePeriod = this.votingConfig.period *
        (1 + (Math.random() - 0.5) * this.votingConfig.period_jitter);
      this.votingConfig.guaranteedResult =
        Math.ceil(this.votingConfig.minVotes * this.votingConfig.supermajority);
      this.voteController = new VoteController(repo,
        this.votingConfig, this.starGazers, this.votesPerPR);
    ...
    Voting.prototype.initialize = function initialize() {
      ...
      if (typeof config.githubAuth === 'undefined') {
        this.intervals.refreshAllPRs = setInterval(this.refreshAllPRs.bind(this),
          this.votingConfig.pollInterval * MINUTE);
      } else {
      ...
      this.intervals.updateStarGazers = setInterval(this.updateStarGazers.bind(this),
        this.votingConfig.pollInterval * MINUTE);
    };
    ...
    Voting.prototype.eventsCommentCreated = function eventsCommentCreated(data) {
      ...
      if (typeof pr === 'undefined') {
        return console.error('Could not find PR', data.issue.number,
     ...
     */
    var updateStarGazers = function updateStarGazers(cb) {
      var self = this;
     ...
     */
    var refreshAllPRs = function refreshAllPRs() {
     ...
     */
    var refreshAllComments = function refreshAllComments(pr, cb) {
    ...
    var handlePR = function handlePR(pr) {
    ...
    var postVoteStarted = function postVoteStarted(pr) {
      ...
      this.getVoteStartedComment(pr, function (err, comment) {
        ...
        makeComment(
          pr.number,
          self.templates.voteStartedComment + " " + pr.head.sha, function (err) {
          if (err) {
          ...
          if (score > 1) {
            ...
            makeComment(pr.number, self.voteController.votePositive);
          } else if (score < -1) {
            // Ugh, this PR sucks, boooo!
            makeComment(pr.number, self.voteController.voteNegative);
     ...
     */
    var getVoteStartedComment = function getVoteStartedComment(pr, cb) {
      for (var i = 0; i < pr.comments.length; i++) {
        var postedByMe = pr.comments[i].user.login === 'anythingbot';
        var isVoteStarted =
          pr.comments[i].body.indexOf(this.templates.voteStartedComment) === 0;
     ...
     */
    var assertNotModified = function assertNotModified(pr, cb) {
     ...
     */
    var processPR = function processPR(pr, cb) {
      ...
      var passes = this.voteController.doesVotePass(voteResults.positive,
        voteResults.negative);
      var endComment = this.templates.voteEndComment(passes, voteResults.positive,
        voteResults.negative, voteResults.nonStarGazers)
    ...
    // close the pull request and post message if provided
    var closePR = function closePR(message, pr, cb) {
      var self = this;
    ...
    // if it can't be merged, post warning and close PR
    var mergePR = function mergePR(pr, cb) {
      var self = this;
      gh.pullRequests.get({
        user: config.user,
        repo: config.repo,
    ...
    Voting.prototype.updateStarGazers = updateStarGazers;
    Voting.prototype.refreshAllPRs = refreshAllPRs;
    Voting.prototype.refreshAllComments = refreshAllComments;
    Voting.prototype.handlePR = handlePR;
    Voting.prototype.postVoteStarted = postVoteStarted;
    Voting.prototype.getVoteStartedComment = getVoteStartedComment;
    Voting.prototype.assertNotModified = assertNotModified;
    Voting.prototype.processPR = processPR;
    Voting.prototype.closePR = closePR;
    Voting.prototype.mergePR = mergePR;
