(function () {
    ...
    define(deps, function (deferred, config, events, eventlog, irc, util, Conversation,
        media) {
      module.exports = function () {
        ...
        if(!config.irc) {
          ...
          console.log("IRC connecting to " + config.irc.host +
            " and joining channel " + config.irc.channel +
            " as " + config.irc.user);
        }
        var ircConfig = {
        ...
        function checkInitNewConversation(nick) {
          eventlog.log('irc.checkInitNewConversation');
        ...
        function buildMessage(state, pr) {
          ...
          return util.format('%s #%d - "%s" - author: @%s - %s',
            lede, pr.number, pr.title, pr.user.login, pr.html_url);
        }
