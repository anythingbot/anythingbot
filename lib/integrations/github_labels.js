(function () {
  ...
  define(deps, function (gh, config, repositories, events, eventlog) {
    module.exports = function () {
      ...
      function eventPullRequestVoteStarted(pr) {
        eventlog.log("github_labels.eventPullRequestVoteStarted");
      ...
      function eventPullRequestClosed(event) {
        eventlog.log("github_labels.eventPullRequestClosed");
      ...
      function eventPullRequestMerged(event) {
        eventlog.log("github_labels.eventPullRequestMerged");
        var pr = event.pull_request;
        var repo = { repo: pr.head.repo.name, user: pr.head.user.login };
      ...
      function getStatusLabel(labelName) {
        eventlog.log("github_labels.getStatusLabel");
      ...
      function setPrStatusLabel(repo, label, pr) {
        eventlog.log("github_labels.setPrStatusLabel");
      ...
      // Filter the pull request labels,
      // only keeping the ones that are no in labelsToRemove
      function removeUnwantedLabels(labelsToRemove, prLabels) {
        eventlog.log('github_labels.removeUnwantedLabels');
      ...
      function applyNewPrLabels(repo, prLabels, pr) {
        eventlog.log('github_labels.applyNewPrLabels');
        gh.issues.edit({
          user: repo.user,
          repo: repo.repo,
          number: pr.number,
      ...
      function ensureRepoHasLabels(repo, requiredLabels, repoLabels) {
        eventlog.log('github_labels.ensureRepoHasLabels');
      ...
      // Returns an array of labels that we want to use,
      // but which do not currently exist in the repo.
